...
- (void)saveStockList:(NSMutableArray*)stock {
    NSUserDefaults *defs = [NSUserDefaults standardUserDefaults];
    NSMutableString *strStockList = [NSMutableString string];
    
    for (int i = 0; i < stock.count; i ++) {
        [strStockList appendString:stock[i]];
        
        if (i != stock.count - 1)
            [strStockList appendString:@","];
    }
    
    [defs setValue:[NSString stringWithString:strStockList] forKey:@"list"];
    [defs synchronize];
}
